...
contract UpgradeScripts is Script {
    ...
    function setUpContract(
        ...
        string memory keyOrContractName = bytes(key).length == 0
            ? contractName
            : key;
        ...
        if (!deployNew) {
            ...
            if (implementation != address(0)) {
                if (implementation.code.length == 0) {
                    console.log(
                        "Stored %s does not contain code.",
                ...
                if (
                    ...
                    console.log(
                        "Implementation for %s changed.",
                        contractLabel(contractName, implementation, key)
                    );

                    if (attachOnly)
                        console.log(
                            "Keeping existing deployment (`attachOnly=true`)."
                        );
                    else deployNew = true;
                }
            } else {
    ...
    function setUpProxy(
        ...
        string memory contractName = registeredContractName[block.chainid][
            implementation
        ];
        string memory keyOrContractName = bytes(key).length == 0
    ...
    function setUpContract(string memory contractName)
        internal
        virtual
        returns (address)
    {
    ...
    function setUpProxy(address implementation, bytes memory initCall)
        internal
        virtual
        returns (address)
    {
    ...
    function loadEnvVars() internal virtual {
        ...
        if (!UPGRADE_SCRIPTS_BYPASS) {
            UPGRADE_SCRIPTS_RESET = tryLoadEnvBool(
                UPGRADE_SCRIPTS_RESET,
                "UPGRADE_SCRIPTS_RESET",
                "US_RESET"
            );
            UPGRADE_SCRIPTS_BYPASS = tryLoadEnvBool(
                UPGRADE_SCRIPTS_BYPASS,
                "UPGRADE_SCRIPTS_BYPASS",
                "US_BYPASS"
            );
            UPGRADE_SCRIPTS_DRY_RUN = tryLoadEnvBool(
    ...
    function upgradeScriptsBroadcast(address account) internal {
        if (!UPGRADE_SCRIPTS_DRY_RUN) {
            ...
            console.log(
                "Make sure you are running this without `--broadcast`."
            );
    ...
    function loadLatestDeployedAddress(string memory key)
        internal
        virtual
        returns (address)
    {
    ...
    function loadLatestDeployedAddress(string memory key, uint256 chainId)
        internal
        virtual
        returns (address)
    {
        if (!__latestDeploymentsLoaded[chainId]) {
            try
                vm.readFile(getDeploymentsPath("deploy-latest.json", chainId))
        ...
        if (bytes(__latestDeploymentsJson[chainId]).length != 0) {
            try
                vm.parseJson(
                    __latestDeploymentsJson[chainId],
                    string.concat(".", key)
                )
    ...
    function loadProxyStoredImplementation(address proxy)
        internal
        virtual
        returns (address)
    {
        ...
        returns (address implementation)
    {
        require(
            proxy.code.length != 0,
            string.concat("No code stored at ", label, ".")
        );
        ...
        try vm.load(proxy, ERC1967_PROXY_STORAGE_SLOT) returns (bytes32 data) {
            ...
            require(
                ...
                string.concat(
                    "Invalid existing implementation address(0) stored in ",
                    label,
                    "."
                )
            );
            require(
                UUPSUpgrade(implementation).proxiableUUID() ==
    ...
    function upgradeSafetyChecks(
        ...
        if (
            isUpgradeSafe[block.chainid][oldImplementation][newImplementation]
        ) {
        ...
        if (diff.length == 0) {
            console.log(
                "Storage layout compatibility check [%s <-> %s]: Pass.",
                oldImplementation,
                newImplementation
            );
        } else {
            console.log(
                "Storage layout compatibility check [%s <-> %s]: Fail",
                oldImplementation,
                newImplementation
            );
            console.log("\n%s Diff:", contractName);
        ...
        isUpgradeSafe[block.chainid][oldImplementation][
            newImplementation
        ] = true;
    }

    function saveCreationCodeHash(address addr, bytes32 creationCodeHash)
        internal
        virtual
    {
    ...
    function creationCodeHashMatches(address addr, bytes32 newCreationCodeHash)
        internal
        virtual
        returns (bool)
    {
    ...
    function fileExists(string memory file)
        internal
        virtual
        returns (bool exists)
    {
    ...
    function assertFileExists(string memory file) internal virtual {
        if (!fileExists(file)) {
            ...
            console.log(
                "You can bypass storage layout comparisons by setting `isUpgradeSafe[..] = true;`."
            );
    ...
    function assertIsERC1967Upgrade(
        address implementation,
        string memory contractName
    ) internal virtual {
        if (implementation.code.length == 0) {
            console.log(
                "No code stored at %s(%s).",
                contractName,
                implementation
            );
        ...
        try UUPSUpgrade(implementation).proxiableUUID() returns (bytes32 uuid) {
            if (uuid != ERC1967_PROXY_STORAGE_SLOT) {
                console.log(
                    "Invalid proxiable UUID for implementation %s(%s).",
                    contractName,
                    implementation
                );
            ...
            console.log(
                "Contract %s(%s) does not implement proxiableUUID().",
                contractName,
                implementation
            );
    ...
    function getContractCode(string memory contractName)
        internal
        virtual
        returns (bytes memory code)
    {
        try vm.getCode(contractName) returns (bytes memory code_) {
            ...
            try vm.getCode(string.concat(contractName, ".sol")) returns (
                bytes memory code_
            ) {
                code = code_;
            } catch {
        ...
        if (code.length == 0) {
            ...
            console.log(
                "Provide either a unique contract name ('MyContract'),"
            );
            console.log(
                "or an artifact location ('MyContract.sol:MyContract')."
            );
    ...
    function requireConfirmation() internal virtual {
        ...
        if (
            block.timestamp - mainnetConfirmation >
            UPGRADE_SCRIPTS_CONFIRM_TIME_WINDOW
        ) {
            revert(
                string.concat(
    ...
    function confirmUpgradeProxy(address proxy, address newImplementation)
        internal
        virtual
    {
        requireConfirmation();
    ...
    function upgradeProxy(address proxy, address newImplementation)
        internal
        virtual
    {
    ...
    function confirmDeployProxy(address implementation, bytes memory initCall)
        internal
        virtual
        returns (address)
    {
    ...
    function confirmDeployCode(bytes memory code)
        internal
        virtual
        returns (address)
    {
    ...
    function deployProxy(address implementation, bytes memory initCall)
        internal
        virtual
        returns (address)
    {
    ...
    function deployCodeWrapper(bytes memory code)
        internal
        virtual
        returns (address addr)
    {
        ...
        require(
            addr.code.length != 0,
            string.concat("Failed to deploy code.", vm.toString(code))
        );
    }
    ...
    function hasCode(address addr)
        internal
        view
        virtual
        returns (bool hasCode_)
    {
        assembly {
    ...
    function registerContract(
        ...
        if (registeredContractAddress[chainId][key] != address(0)) {
            console.log(
                "Duplicate entry for key [%s] found when registering contract.",
                key
            );
            console.log(
                "Found: %s(%s)",
                key,
                registeredContractAddress[chainId][key]
            );
    ...
    function logDeployments() internal view virtual {
        ...
        for (uint256 i; i < registeredChainIds.length(); i++) {
            ...
            for (uint256 j; j < registeredContracts[chainId].length; j++) {
                console.log(
                    "%s=%s",
                    registeredContracts[chainId][j].key,
                    registeredContracts[chainId][j].addr
                );
            }
    ...
    function generateRegisteredContractsJson(uint256 chainId)
        internal
        virtual
        returns (string memory json)
    {
        ...
        if (!UPGRADE_SCRIPTS_DRY_RUN) {
            for (uint256 i; i < registeredChainIds.length(); i++) {
                ...
                if (
                    ...
                    keccak256(bytes(__latestDeploymentsJson[chainId]))
                ) {
                    ...
                    vm.writeFile(
                        getDeploymentsPath(
                            string.concat("deploy-latest.json"),
                            chainId
                        ),
                        json
                    );
    ...
    function getDeploymentsPath(string memory path, uint256 chainId)
        internal
        virtual
        returns (string memory)
    {
    ...
    function getCreationCodeHashFilePath(address addr)
        internal
        virtual
        returns (string memory)
    {
        return
            getDeploymentsPath(
                string.concat("data/", vm.toString(addr), ".creation-code-hash")
            );
    }
